...
public class UserController {
    ...
    private static final String REGISTER_CLIENT_URL = "/client/register";
    ...
    @Operation(
            summary = CREATE_CLIENT_SUMMARY,
            description = CREATE_CLIENT_DESCRIPTION,
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = CREATE_CLIENT_REQUEST_BODY_DESCRIPTION,
                    content = @Content(
                            schema = @Schema(implementation = UserRequest.class),
                            examples = {
                                    @ExampleObject(value = USER_REQUEST_EXAMPLE)
    ...
    @PostMapping(REGISTER_CLIENT_URL)
    @PreAuthorize("permitAll()")
    ResponseEntity<AuthResponse> registerClient(@Valid @RequestBody UserRequest userRequest) {
        AuthResponse authResponse = userHandler.createUserClient(userRequest);
